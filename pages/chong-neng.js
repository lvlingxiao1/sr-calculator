...
const 测试回能 = (
	...
	返回文本.push(
		`三战技：${三战技.toFixed(2)} ${
			三战技 > 能量需求
				? '已充满'
				: `离充满差${(能量需求 - 三战技).toFixed(2)}`
		}`
	);
	const 两战技一普攻 =
		(5 + 战技回能 * 2 + 普通攻击回能 + 每回合额外回能 * 3) * 充能效率;
	返回文本.push(
		`两战技一普攻：${两战技一普攻.toFixed(2)} ${
			两战技一普攻 > 能量需求
				? '已充满'
				: `离充满差${(能量需求 - 两战技一普攻).toFixed(2)}`
		}`
	);
	const 一战技两普攻 =
		(5 + 战技回能 + 普通攻击回能 * 2 + 每回合额外回能 * 3) * 充能效率;
	返回文本.push(
		`一战技两普攻：${一战技两普攻.toFixed(2)} ${
			一战技两普攻 > 能量需求
				? '已充满'
				: `离充满差${(能量需求 - 一战技两普攻).toFixed(2)}`
		}`
	);
	...
	返回文本.push(
		`两战技：${两战技.toFixed(2)} ${
			两战技 > 能量需求
				? '已充满'
				: `离充满差${(能量需求 - 两战技).toFixed(2)}`
		}`
	);
	const 一战技一普攻 =
		(5 + 战技回能 + 普通攻击回能 + 每回合额外回能 * 2) * 充能效率;
	返回文本.push(
		`一战技一普攻${一战技一普攻.toFixed(2)} ${
			一战技一普攻 > 能量需求
				? '已充满'
				: `离充满差${(能量需求 - 一战技一普攻).toFixed(2)}`
		}`
	);
		...
		<>
			<div>
				...
				<div>
					...
					<input
						type="number"
						onChange={(e) => 设置战技回能(+e.target.value)}
						value={战技回能}
					></input>
				</div>
				<div>
				...
				<div>
					...
					<input
						type="number"
						onChange={(e) => 设置能量需求(+e.target.value)}
						value={能量需求}
					></input>
				</div>
			</div>
			...
			<h3>
				...
				<input
					type="number"
					onChange={(e) => 设置充能效率(+e.target.value)}
					value={充能效率}
				></input>
				%）
			</h3>
