...
export default function 青雀抽牌页面() {
    ...
    const 胡了 = (手牌) => {
        if (手牌.length === 4 && 手牌.reduce((pre, curr, i, array) => { return pre && curr === array[0] }, true)) {
            return true;
        }
        return false;
    }
    ...
    const 计算图表 = () => {
        ...
        const pieData = [无不求人统计.slice(0, 6).reduce((pre, curr) => pre + curr, 0), 有不求人统计.reduce((pre, curr) => pre + curr, 0), 无不求人统计[6]]
        ...
        设置图表(<div style={{ width: "70rem" }}>
            < Bar options={options1} data={{ labels: labels, datasets: [{ label: '无不求人', data: 无不求人统计, backgroundColor: 'rgba(255, 99, 132, 0.5)', }, { label: '有不求人', data: 有不求人统计, backgroundColor: 'rgba(53, 162, 235, 0.5)', }] }} />
            < Pie options={options2} data={{ labels: pieLabels, datasets: [{ label: '次数', data: pieData, backgroundColor: ['rgba(255, 99, 132, 0.5)', 'rgba(53, 162, 235, 0.5)', 'rgba(100, 255, 100, 0.5)'], }] }} />
            < Pie options={options3} data={{
                labels: 战技点标签, datasets: [{
                    label: '次数', data: 战技点数据, backgroundColor: [
                        'rgba(40, 252, 111)',
                        'rgba(35, 219, 97)',
                        'rgba(28, 173, 77)',
                        'rgba(20, 120, 54)',
